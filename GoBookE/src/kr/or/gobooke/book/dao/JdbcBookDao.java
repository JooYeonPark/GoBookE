...
public class JdbcBookDao implements BookDao {
	...
	public List<Book> getlistStock(BookParams params) {
		...
		if(type != null){
			switch (params.getType()) {
				case "title":    
					...
					break;
				case "author":  
					sb.append(" WHERE  book_author LIKE ?");
					value = "%" + value + "%";
					break;
			}
		}
		sb.append("                ORDER  BY book_qty ASC))"); 
		sb.append("WHERE  request_page = ?"); 
		...
		try {
			...
			e.printStackTrace();
			throw new MallException("JdbcUserDao.getlistStock(Params params) 실행 중 예외발생", e);
		} finally {
			try {
				if(rs != null)    rs.close();
				if(pstmt != null) pstmt.close();
	...
	public List<String> getPublisher() {
		...
		String sql = "SELECT DISTINCT book_publisher  " + 
					 " FROM   book"; 
		...
		try {
			...
			while (rs.next()) {
				String val = rs.getString("book_publisher");
				publisher.add(val);
			}
			...
			throw new MallException("JDBCBookDao.getPublisher() 실행중 예외 발생", e);
		}
